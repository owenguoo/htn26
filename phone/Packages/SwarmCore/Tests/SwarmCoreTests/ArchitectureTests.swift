...
struct ArchitectureTests {
    ...
    @Test func javascriptNeverOpensASocket() throws {
        ...
        // Not a silent pass: if the Expo app moves, this test must fail rather
        // than quietly assert nothing for the rest of its life.
        #expect(FileManager.default.fileExists(atPath: mobile.path),
                "mobile/ has moved — this test was asserting nothing")
        ...
        for directory in ["app", "src", "modules/beacon/src", "modules/beacon/index.ts", "App.tsx"] {
            ...
            for file in files {
                ...
                // "WebSocket" alone caught only the most literal violation.
                // The rule is that JS does not talk to the hub and does not
                // poll native at rate, so the other shapes of the same mistake
                // are named here: an HTTP client to the hub, and a timer that
                // turns a human-rate API into a high-frequency one.
                let needles = ["WebSocket", "/ws/phone", "EventSource",
                               "XMLHttpRequest", "fetch(", "setInterval("]
                for needle in needles where source.contains(needle) {
    ...
    /// The other half of the same rule, which a grep over JS cannot see: the
    /// native module's own event surface. A `sendEvent` per pose or per frame
    /// would satisfy every assertion above while putting exactly the traffic
    /// the rule forbids across the bridge, so the event list and the pump's
    /// interval are pinned here.
    @Test func theNativeModuleEmitsOnlyHumanRateEvents() throws {
        let module = repositoryRoot
            .appendingPathComponent("mobile/modules/beacon/ios/Expo/BeaconModule.swift")
        guard FileManager.default.fileExists(atPath: module.path) else { return }
        let source = try String(contentsOf: module, encoding: .utf8)

        #expect(source.contains(#"Events("onState", "onWelcome", "onPhase", "onError")"#),
                "the JS event surface changed — check the new event's rate and payload")

        // The snapshot pump. 500 ms is 2 Hz; anything under 250 ms is no longer
        // a human rate and does not belong on the bridge.
        let interval = source
            .components(separatedBy: "try? await Task.sleep(nanoseconds: ")
            .dropFirst()
            .compactMap { UInt64($0.prefix { $0.isNumber || $0 == "_" }.replacingOccurrences(of: "_", with: "")) }
        #expect(!interval.isEmpty, "could not find the snapshot pump's sleep")
        #expect(interval.allSatisfy { $0 >= 250_000_000 },
                "a native→JS pump is running faster than 4 Hz: \(interval)")
    }
