...
while IFS='=' read -r key value; do
  case "$key" in MAP_WORKER_*) export "$key=$value" ;; esac
done < .env
...
case "${1:-status}" in
  start)
    run "mkdir -p $DIR"
    scp -q -o BatchMode=yes -P "$PORT" gpu/vggt_worker.py gpu/surface.py gpu/mesh_glb.py gpu/requirements-surface.txt "$HOST:$DIR/"
