...
probability map, detection model and planner as the hub. Modes:
    ...
    plan    the planner: most probability of finding per second, walking when it's worth it
...
Control turning "last seen near X" into likelihood.
    ...
    uv run python scripts/bench_planner.py plan uniform 40
    uv run python scripts/bench_planner.py plan prior 40
"""
...
def run(mode, seed, prior=False):
    ...
    cov = Coverage(ROOM); pl = Planner(ROOM, cov); pl.enabled = mode != "none"
