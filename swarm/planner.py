...
Two modes:
- "bayes" (default), from Bayesian search theory (how the Coast Guard plans searches): every
  option is scored by how much probability of finding the candidate it's expected to buy per
...
- "greedy": the original rule, most unsearched floor in reach (kept to compare against).
...
class Planner:
    ...
    def tick(self, viewers: dict[str, tuple[float, float, float, float | None]], now: float) -> list[tuple[str, dict]]:
        ...
        # finished or stalled sectors (greedy mode; Bayesian looks are re-scored every plan instead)
        ...
        if now - self.last_guide >= GUIDE_EVERY_MS:
            ...
            for pid, a in self.assignments.items():
                ...
                if a.get("heading") is not None:  # Bayesian looks aim at a direction, not a sector center
    ...
    def plan_bayes(self, viewers: dict, now: float) -> None:
    ...
    def snapshot(self) -> dict:
        return {
            "enabled": self.enabled, "mode": self.mode, "sectorSize": SECTOR,
