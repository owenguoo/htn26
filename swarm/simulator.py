...
class Environment:
    ...
    def first_collision(self, a: tuple[float, float], b: tuple[float, float]) -> tuple[float, int] | None:
        """First blocked cell along a segment, with its entry fraction in [0, 1].

        Traverse grid boundaries exactly, including both side cells at diagonal
        corners. Checking only the endpoint can skip thin walls or doorways.
        Outside the map is blocked and has cell id -1.
        """
        col, row = math.floor(a[0] / self.cell), math.floor(a[1] / self.cell)

        def blocked(c, r):
            if not (0 <= c < self.cols and 0 <= r < self.rows):
                return -1
            i = r * self.cols + c
            return i if not self.walkable[i] else None
        ...
        tx = (((col + (dx > 0)) * self.cell - a[0]) / dx) if dx else math.inf
        ty = (((row + (dy > 0)) * self.cell - a[1]) / dy) if dy else math.inf
        dtx, dty = self.cell / abs(dx) if dx else math.inf, self.cell / abs(dy) if dy else math.inf
        while min(tx, ty) <= 1.0:
            t = min(tx, ty)
            cross_x, cross_y = tx <= ty + 1e-12, ty <= tx + 1e-12
            cells = []
            if cross_x:
                cells.append((col + sx, row))
            if cross_y:
                cells.append((col, row + sy))
            if cross_x and cross_y:
                cells.append((col + sx, row + sy))
            for c, r in cells:
                hit = blocked(c, r)
                if hit is not None:
                    return max(0.0, t), hit
            if cross_x:
                col += sx
                tx += dtx
            if cross_y:
    ...
    def path_from(self, x: float, y: float, target: int) -> list[tuple[float, float]]:
        """Waypoints from an actual position, which need not be a cell center."""
        path = self.path(self.cell_at(x, y), target)
        if not path:
            return []
        # A path planned from the center is not necessarily safe from the crew's
        # current sub-cell position, especially when retasked beside a corner.
        if len(path) > 1 and self.first_collision((x, y), path[1]) is None:
            return path[1:]
        return path if math.dist((x, y), path[0]) > 1e-9 else path[1:]
...
def move_along(env: Environment, a, dt: float, speed: float) -> int | None:
    """Walk only collision-free segments; return the first obstacle cell, if any.

    A collision consumes only the distance actually walked and keeps remaining
    waypoints for the caller to replan or report. Pass physical geometry here,
    even when the route was planned using an incomplete map.
    """
    ...
    while a.path and budget > 1e-9:
        ...
        collision = None
        if d > 1e-6:
            ux, uy = (tx - a.x) / d, (ty - a.y) / d
            collision = env.first_collision((a.x, a.y), (a.x + ux * step, a.y + uy * step))
            if collision is not None:
                step = max(0.0, step * collision[0] - env.cell * 1e-7)
            a.x += ux * step
            a.y += uy * step
        a.walked += step
        budget -= step
...
class Run:
    ...
    def assign(self, i: int, option: dict, worth: np.ndarray) -> None:
        ...
        if option["kind"] == "walk":
            a.path = env.path_from(a.x, a.y, option["origin"])
    ...
    def found(self, i: int) -> None:
        ...
        for k, a in enumerate(self.team):
            ...
            if k in self.responders:
                a.path = env.path_from(a.x, a.y, self.victim)
